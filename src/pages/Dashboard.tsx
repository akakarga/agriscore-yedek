...
import { Users, AlertTriangle, TrendingUp, DollarSign, ShieldAlert, MapPin, Milk, CheckCircle2, ChevronRight, Activity, X } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const priorityCandidates = useMemo(() => {
    ...
    return [...stats.scoredProducers]
      .filter(p => p.score.overallScore >= minScore && p.financials.requestedLoanAmount >= 500000)
      .sort((a, b) => b.financials.requestedLoanAmount - a.financials.requestedLoanAmount);
  ...
  const openScenario = (type: string) => {
    if (type === 'feed') {
      setShockScenario({
        title: "Yem Maliyetleri %20 Artarsa",
        desc: "İşletmelerin aylık giderleri %20 artırılmış ve mevcut gelirlerle borç çevirme oranları yeniden simüle edilmiştir.",
        impacts: [
          "Ortalama Portföy Skoru: 72 ➔ 64",
          "Orta Riskli Üreticilerin %30'u Yüksek Riske geçiş yaptı",
          "Kredi Geri Dönüş Gecikme Beklentisi: %1.2 ➔ %4.5"
        ]
      });
    } else if (type === 'milk') {
      setShockScenario({
        title: "Çiğ Süt Fiyatı %10 Düşerse",
        desc: "Aylık süt gelirleri %10 azaltılmış ve işletmelerin nakit akışı stres testine tabi tutulmuştur.",
        impacts: [
          "Ortalama Portföy Skoru: 72 ➔ 68",
          "Nakit Akışı Negatife Düşen İşletme: 4 Aday",
          "Yeni Kredi Taleplerinde %25 Artış Beklentisi"
        ]
      });
    } else if (type === 'disease') {
      setShockScenario({
        title: "Ulusal Hastalık (Şap) Senaryosu",
        desc: "Bölgesel veya ulusal boyutta salgın hastalık olması durumunda sürü verimliliği ve karantina etkileri.",
        impacts: [
          "Süt Veriminde Portföy Bazında %15 Düşüş",
          "Veterinerlik ve İlaç Maliyetlerinde %40 Artış",
          "Genel Portföy Risk Notu: ORTA ➔ YÜKSEK"
        ]
      });
    }
  };
  ...
  return (
    <div className="tour-dashboard space-y-6">
      ...
      {/* KPI Cards Row 1 */}
      <div className="tour-kpi-cards grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="tour-risk-chart bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Policy Simulator */}
        <div className="tour-simulation bg-gradient-to-br from-fin-900 to-slate-800 rounded-xl p-6 text-white shadow-sm border border-fin-800 relative overflow-hidden">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span>Mevcut Politika: {getPolicyLabel(policyLevel)}</span>
                <span className="text-agri-400 font-bold">Öncelikli: {priorityCandidates.length} Aday</span>
              </div>
              <input type="range" min="1" max="3" value={policyLevel} onChange={(e) => setPolicyLevel(Number(e.target.value))} className="w-full h-2 bg-slate-600 rounded-lg appearance-none cursor-pointer" />
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm relative overflow-hidden">
          ...
          <div className="space-y-3">
            <button onClick={() => openScenario('feed')} className="w-full flex items-center justify-between p-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
              <span className="text-sm font-medium text-slate-700">Yem Maliyetleri %20 Artarsa</span>
              <ChevronRight className="w-4 h-4 text-slate-400" />
            </button>
            <button onClick={() => openScenario('milk')} className="w-full flex items-center justify-between p-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
              <span className="text-sm font-medium text-slate-700">Çiğ Süt Fiyatı %10 Düşerse</span>
              <ChevronRight className="w-4 h-4 text-slate-400" />
            </button>
            <button onClick={() => openScenario('disease')} className="w-full flex items-center justify-between p-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
              ...
              <div className="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden animate-fade-in">
                <div className="bg-orange-50 p-4 border-b border-orange-100 flex justify-between items-start">
                  <div className="flex items-center">
                    ...
                    <h4 className="font-bold text-orange-900">{shockScenario.title}</h4>
                  </div>
                ...
                <div className="p-6">
                  <p className="text-sm text-slate-600 mb-4 pb-4 border-b border-slate-100">{shockScenario.desc}</p>
                  <h5 className="font-semibold text-slate-800 mb-3 text-sm">Simülasyon Sonuçları:</h5>
                  ...
                  <button onClick={() => setShockScenario(null)} className="mt-6 w-full py-2 bg-fin-900 text-white rounded-lg hover:bg-fin-800 transition-colors text-sm font-medium">
                    Anladım, Kapat
                  </button>
                </div>
              </div>
